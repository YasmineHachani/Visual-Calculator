...
def extract_feature(input_image): 
    image =  (255*input_image).astype('uint8')
...
def create_df(image):
    (wristX, wristY, wristZ,
                ...
                pinky_DipX, pinky_DipY, pinky_DipZ,
                pinky_TipX, pinky_TipY, pinky_TipZ,
                annotated_image) = extract_feature(image)
    df = np.array([wristX, wristY, wristZ,
                thumb_CmcX, thumb_CmcY, thumb_CmcZ,
                thumb_McpX, thumb_McpY, thumb_McpZ,
                thumb_IpX, thumb_IpY, thumb_IpZ,
                thumb_TipX, thumb_TipY, thumb_TipZ,
...
#Creating window to show calculation
def Text_Window(image, text, x, y):
    
    fontscale = 1.0
    color = (0, 0, 0)
    fontface = cv2.FONT_HERSHEY_COMPLEX_SMALL
    cv2.putText(image, str(text), (x, y), fontface, fontscale, color)
